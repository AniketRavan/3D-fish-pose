function [eye1_model,eye1_c] = eye1model(x, y, z, seglen, brightness, size_lut, rnd)
d_eye = seglen * (0.8456 + (rnd(1) - 0.5)*0.05);% 1.4332; %0.83
c_eyes = 1.4130; %1.3015;
eye1_w = seglen * (0.2097 + (rnd(2) - 0.5)*0.02); %0.1671; % 0.35
eye1_w = seglen * (0.2597 + (rnd(2) - 0.5)*0.02);
eye1_w = seglen * (0.3297 + (rnd(2) - 0.5)*0.02);
eye1_l = seglen * 0.3306; %0.2507; % 0.45 3006
eye1_l = seglen * (0.4756 + (rnd(3) - 0.5)*0.02);
eye1_h = seglen * (0.2496 + (rand - 0.5)*0.02); %0.2661; % 0.35
eye1_h = seglen * (0.2996 + (rnd(4) - 0.5)*0.02);
