function [eye2_model,eye2_c] = eye2model(x, y, z, seglen, brightness, size_lut, rnd)
d_eye = seglen * (0.8456 + (rnd(1) - 0.5)*0.05); % 1.4332; %0.83
c_eyes = 1.4130; %1.3015;
eye2_w = seglen * (0.2097 + (rnd(2) - 0.5)*0.02); %0.1671; % 0.35
eye2_w = seglen * (0.2597 + (rand - 0.5)*0.02);
eye2_w = seglen * (0.3297 + (rnd(2) - 0.5)*0.02);
eye2_l = seglen * (0.3306 + (rnd - 0.5)*0.02); %0.2507; % 0.45
eye2_l = seglen * (0.4756 + (rnd(3) - 0.5)*0.02);
eye2_h = seglen * (0.2496 + (rand - 0.5)*0.02); %0.2661; % 0.35
eye2_h = seglen * (0.2996 + (rnd(4) - 0.5)*0.02);
