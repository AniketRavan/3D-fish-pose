...
def activation_func(activation):
    return nn.ModuleDict([['relu',nn.ReLU(inplace=True)],['leaky_relu',nn.LeakyReLU(negative_slope=0.01, inplace=True)],
        ['selu',nn.SELU(inplace=True)],['sigmoid',nn.Sigmoid()],['none',nn.Identity()]])[activation]
...
class ResnetDecoder(nn.Module):
    ...
    def __init__(self, in_features, n_classes):
        ...
        self.bn_crop0 = nn.BatchNorm1d(n_classes)
        self.bn_crop1 = nn.BatchNorm1d(12*n_classes)
        self.bn_crop2 = nn.BatchNorm1d(24*n_classes)
        self.FC_crop0 = nn.Linear(6, n_classes)
        self.FC_crop1 = nn.Linear(n_classes, 12*n_classes)
        self.FC_crop2 = nn.Linear(12*n_classes, 24*n_classes)
        self.bn0 = nn.BatchNorm1d(24*n_classes)
        self.bn0_crop_added = nn.BatchNorm1d(24*n_classes)
        self.FC1 = nn.Linear(24*n_classes, 12*n_classes)
        self.bn1 = nn.BatchNorm1d(12*n_classes)
        self.FC2 = nn.Linear(12*n_classes, 3*n_classes)
    ...
    def forward(self, x, crop_coor_data):
        x = self.avg(x)
        x = x.view(x.size(0), -1)
        x = self.leaky_relu(self.decoder(x))
        x = self.bn0(x)
        crop_intermediate1 = self.bn_crop0(self.leaky_relu(self.FC_crop0(crop_coor_data)))
        crop_intermediate2 = self.bn_crop1(self.leaky_relu(self.FC_crop1(crop_intermediate1)))
        x = x + self.bn_crop2(self.leaky_relu(self.FC_crop2(crop_intermediate2)))
