...
model.load_state_dict(torch.load('resnet_pose_220714_1.pt'))
date = '220716'
transform = transforms.Compose([transforms.ToTensor(), transforms.ConvertImageDtype(torch.float)])
#im_folder = '../validation_data_3D_pose_shifted/images_gray/'
im_folder = '../lookup_table_head/training_data_3D_' + date + '/images_real/'
im_files = sorted(os.listdir(im_folder))
im_files_add = [im_folder + file_name for file_name in im_files]

pose_folder = '../lookup_table_head/training_data_3D_' + date + '/annotations_220714_pose_tensor/'
pose_files = sorted(os.listdir(pose_folder))
pose_files_add = [pose_folder + file_name for file_name in pose_files]

eye_coor_folder = '../lookup_table_head/training_data_3D_' + date + '/annotations_220714_eye_coor_tensor/'
eye_files = sorted(os.listdir(eye_coor_folder))
eye_coor_files_add = [eye_coor_folder + file_name for file_name in eye_files]

#crop_coor_folder = '../validation_data_3D_pose_shifted/annotations_220713_crop_coor_tensor/'
crop_coor_folder = '../lookup_table_head/training_data_3D_' + date + '/annotations_220714_crop_coor_tensor/'
...
val_data = CustomImageDataset(im_files_add, pose_files_add, eye_coor_files_add, crop_coor_files_add, transform=transform)
             ...
             #          axs[0,m].scatter(eye_coor_data[m,0,0:2].cpu(), eye_coor_data[m,1,0:2].cpu(), s=0.07, c='green', alpha=0.6)
              ...
              #         axs[2,m].scatter(eye_coor_data[m,0,2:4].cpu(), eye_coor_data[m,1,2:4].cpu(), s=0.07, c='green', alpha=0.6)
               ...
               #        axs[4,m].scatter(eye_coor_data[m,0,4:6].cpu(), eye_coor_data[m,1,4:6].cpu(), s=0.07, c='green', alpha=0.6)
                    ...
                    plt.savefig(output_dir + "/epoch_" + str(i) + ".svg")
                    plt.close()
