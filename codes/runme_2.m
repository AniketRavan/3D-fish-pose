...
for path_idx = 1
    ...
    for z = round(((length(coor_mf_mats)) - 2)/4):round(2*(length(coor_mf_mats) - 2)/4)
...
%                     imwrite(im_tail,['training_side_tail\im_',num2str(idx),'_',num2str(n),'.png'])
		    ...
		    save(['../training_data_3D_pose_new/annotations_220623_eye_coor/eye_coor_ann_2',num2str(idx),'.mat'],  'eye_coor');
                    idx = idx + 1;
		    if (idx == 200000)
			display('Finished 200000')
			return
		    end
                end
            end
